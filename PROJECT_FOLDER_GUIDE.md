...
### `composer.json`
Composer metadata for the project. It declares the project name, description, PHP version requirement, PHPUnit development dependency, and PSR-4 autoloading for `App\` and `Tests\`.

### `composer.lock`
Composer dependency lock file. It records the exact installed package versions, including PHPUnit, so teammates can install the same dependency set with `composer install`.
...
### `phpunit.xml`
PHPUnit configuration file. It loads `vendor/autoload.php`, points PHPUnit to `tests/Unit`, enables colored output, and marks `app` as the source code folder for test reporting.
...
### `vendor`
Composer-installed dependencies. This folder is generated by `composer install` and includes `vendor/bin/phpunit`, the command used to run the unit test suite.
...
Custom runtime autoloader for project classes under the `App` namespace, plus `app/Core/helpers.php`. Composer autoloading is also configured for PHPUnit and other Composer-based tooling.
...
| `Diagrams/Database` | Database ERD PNG exports, split by feature area and full schema. |
...
Current database diagram exports:

| File | Purpose |
| --- | --- |
| `Core-Jobs-Applications ERD.png` | Core users, jobs, candidates, and applications ERD. |
| `Screening-Assessments ERD.png` | Screening, shortlist, duplicate detection, and assessment ERD. |
| `Interviews-Panel-Workspace ERD.png` | Interview scheduling, panel assignment, and workspace ERD. |
| `Final Evaluation-Offers-Audit-Compliance ERD.png` | Final evaluation, offers, audit, retention, and compliance ERD. |
| `Full-schema ERD.png` | Full database schema ERD export. |
...
PHPUnit tests live in the `tests` folder. The current suite is focused on fast unit tests that do not require a browser or a real database connection.

Run the tests from Git Bash with:

```bash
vendor/bin/phpunit
```

Run the tests from PowerShell with:
...
Unit tests check individual classes, methods, and business rules in isolation.

| File | Purpose |
| --- | --- |
| `ApplicationPolicyTest.php` | Tests HR-only application status transition permissions. |
| `FeedbackNormalizationServiceTest.php` | Tests feedback aggregate scoring, fallback behavior, and normalization. |
| `NotificationPolicyTest.php` | Tests notification view and mark-read permissions. |
| `OfferPackageCalculatorTest.php` | Tests compensation package calculations, fallback offer type behavior, overrides, and salary suggestions. |
| `SimulatedMatchScorerTest.php` | Tests candidate/job match scoring and weighted skill breakdowns. |
| `TemplateVersionDiffServiceTest.php` | Tests template/version text diff output for added, removed, and unchanged lines. |
| `ValidatorTest.php` | Tests required fields, email validation, trimming, and optional empty fields. |
