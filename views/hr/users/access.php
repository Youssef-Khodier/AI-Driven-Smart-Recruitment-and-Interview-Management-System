...
<form method="POST" action="<?= e(url('hr.users.access.update', [$target['user_id']])) ?>">
    ...
    <label>Role<select name="role"><?php foreach (\App\Enums\UserRole::values() as $role): ?><option value="<?= e($role) ?>"<?= selected($target['role'], $role) ?>><?= e($role) ?></option><?php endforeach; ?></select></label>
